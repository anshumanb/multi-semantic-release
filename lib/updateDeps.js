...
const needsDependencyUpdate = (pkg, dependency, bumpStrategy) => {
	...
	//Verify peer dependency includes version being released
	const currentVersionRange = peerDependencies[dependency.name];
	if (currentVersionRange) {
		const doesPeerDepSatisfy =
			semver.validRange(currentVersionRange) && semver.satisfies(depNextVersion, currentVersionRange);
		if (!doesPeerDepSatisfy) {
			throw Error(
				`Peer dependency ${dependency.name} range ${currentVersionRange} does not include new version ${depNextVersion}`
			);
		} else {
			return false;
		}
...
const manifestUpdateNecessary = (scope, name, nextVersion, bumpStrategy) => {
	...
	if (!nextVersion || !currentVersion || currentVersion === nextVersion) {
		return false;
	}

	if (!["inherit", "satisfy"].includes(bumpStrategy)) {
		// override
		return true;
	}

	return !semver.satisfies(nextVersion, currentVersion);
};
...
const resolveNextVersion = (currentVersion, nextVersion, bumpStrategy = "override") => {
	...
	if (bumpStrategy === "inherit") {
		...
		// prettier-ignore
		const resolvedChunks = nextChunks.map((chunk, i) =>
			currentChunks[i]
				? currentChunks[i].replace(/\d+/, chunk)
				: chunk
		);
 ...
 * @param {string} bumpStrategy The bump strategy
 * @param {boolean} writeOut Commit the package to the file store (set to false to suppres)
 * @returns {undefined}
 * @internal
 */
const updateManifestDeps = (pkg, bumpStrategy, writeOut = true) => {
	const { manifest, path } = pkg;

	let isAnyDepUpdated = false;
	...
	pkg._depsChanged.forEach((dependency) => {
		...
		scopes.forEach((scope) => {
			const currentVersion = scope[dependency.name];
			if (currentVersion) {
				scope[dependency.name] = resolveNextVersion(currentVersion, release.version, bumpStrategy);
				if (currentVersion !== scope[dependency.name]) {
					isAnyDepUpdated = true;
				}
			}
		});
	});
